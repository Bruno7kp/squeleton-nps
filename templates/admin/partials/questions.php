<section>
    ...
    <div class="d-flex f-col f-gap-20">
        ...
        <div class="card p-15-all">
            ...
            <form class="d-flex f-col f-gap-10 m-10-b" hx-post="/admin/rules" hx-target="#admin-content" hx-swap="innerHTML">
                <?= \App\Support\Csrf::hiddenInput() ?>
                <input type="hidden" name="survey_id" value="<?= (int) ($survey['id'] ?? 0) ?>">

                <label class="d-flex f-col f-gap-5" for="rule-source-question">
                    <span class="fw-600">Quando pergunta</span>
                    <select id="rule-source-question" class="form-control w-100" name="source_question_id" required>
                ...
                <div class="row gap-10">
                    <div class="c-xs-12 c-md-4">
                        <label class="d-flex f-col f-gap-5" for="rule-operator">
                            <span class="fw-600">Operador</span>
                            <select id="rule-operator" class="form-control w-100" name="operator" required>
                    ...
                    <div class="c-xs-12 c-md-4">
                        <label class="d-flex f-col f-gap-5" for="rule-compare-value">
                            <span class="fw-600">Valor</span>
                            <input id="rule-compare-value" class="form-control w-100" type="text" name="compare_value" required>
                        </label>
                    </div>
                    <div class="c-xs-12 c-md-4">
                        <label class="d-flex f-col f-gap-5" for="rule-action">
                            <span class="fw-600">Ação</span>
                            <select id="rule-action" class="form-control w-100" name="action" required>
                ...
                <label class="d-flex f-col f-gap-5" for="rule-target-question">
                    <span class="fw-600">Mostrar pergunta</span>
                    <select id="rule-target-question" class="form-control w-100" name="target_question_id" required>
                        <option value="">Selecione...</option>
                        <?php foreach ($questions as $question): ?>
                            <option value="<?= (int) $question['id'] ?>"><?= htmlspecialchars((string) $question['field_name'], ENT_QUOTES, 'UTF-8') ?> - <?= htmlspecialchars((string) $question['label'], ENT_QUOTES, 'UTF-8') ?></option>
                        <?php endforeach; ?>
                        ...
                        <?php else: ?>
                            <?php foreach ($rules as $rule): ?>
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <form class="m-0-b" hx-post="/admin/rules/<?= (int) $rule['id'] ?>/delete" hx-target="#admin-content" hx-swap="innerHTML">
                                            <?= \App\Support\Csrf::hiddenInput() ?>
                                            <input type="hidden" name="survey_id" value="<?= (int) ($survey['id'] ?? 0) ?>">
